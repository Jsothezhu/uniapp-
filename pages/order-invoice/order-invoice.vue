<template>
	<view>
		<divider></divider>
		<card headTitle="发票类型" bodyPadding cardStyle="background:#fff">
			<zcmradioGroup :label="label1" 
			:selected.sync='label1.selected'></zcmradioGroup>
			<view class="text-light-muted line-h-md">
				电子发票与纸质发票具有相同的法律效力，可作为报销、售后、维权凭证，
				推荐使用电子发票，不怕丢失，更方便，环保
			</view>
		</card>
		<divider></divider>
		<card headTitle="发票抬头" bodyPadding cardStyle="background:#fff">
			<zcmradioGroup :label="label2" 
			:selected.sync='label2.selected'></zcmradioGroup>
		</card>
		<divider></divider>
		...
		<view class="p-2 text-light-muted d-flex flex-column">
			<text>发票须知：</text>
			<text>1.发票我为实际支付金额，不包括优惠卷等；</text>
