<template>
	<view class="bg-white rounded border d-flex a-center mt-3"
	@click="$emit('click')">
		<view class="flex-1 d-flex flex-column j-center p-2">
			<view style="font-size: 38rpx;" :class="item.disabled || !item.status ? 'text-light-muted' :''">
				{{item.title}}
			</view>
		...
		<view class="text-white d-flex flex-column a-center j-center border-light-secondary"
			style="width: 220rpx;height: 200rpx;"
			:class="item.disabled || !item.status ? 'bg-secondary' : 'main-bg-color'">
			<view style="font-size: 40rpx; line-height: 1.4;">{{item.price}}
			<text class="font">{{item.type ===0 ?'元' : '折'}}</text></view>
